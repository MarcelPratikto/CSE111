...
def print_marriages(marriages_dict, people_dict):
    ...
    for key, value in marriages_dict.items():
        ...
        print(f"Marriage #{count}")
        print(f"Husband's Name: {husband_name}")
        print(f"Husband's Age: {husband_age}")
        print(f"Wedding Year: {wedding_year}")
        print(f"Wife's Name: {wife_name}")
        print(f"Wife's Age: {wife_age}")
        print()
        count += 1

def count_marriages(marriages_dict, people_dict):
    """
    Counts & prints the number of marriages that each person had
    Prints who married the most times
    Parameters
        marriages_dict: a dictionary that contains data about
            marriages. Each item in the dictionary is in this format:
            marriage_key: [husband_key, wife_key, wedding_year]
    ...
    """
    id_married_most = ""
    num_marriages_record = 0
    num_marriages = {}
    for key, value in marriages_dict.items():
        # count husband
        if value[0] in num_marriages:
            num_marriages[value[0]] += 1
        else:
            num_marriages[value[0]] = 1
        # count wife
        if value[1] in num_marriages:
            num_marriages[value[1]] += 1
        else:
            num_marriages[value[1]] = 1
    #print(num_marriages)
    # print who got married the most
    for person, marriages in num_marriages.items():
        if marriages > num_marriages_record:
            num_marriages_record = marriages
            id_married_most = person
    name_married_most = people_dict[id_married_most][0]
    print(f"{name_married_most} got married a record amount of {num_marriages_record} times")
